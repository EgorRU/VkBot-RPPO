...
async def main(message: Message):
    ...
    base.commit()
    active = int(cur.execute(f"SELECT active FROM registration_data WHERE id={message.peer_id}").fetchone()[0])
    base.close()
...
async def source_vyatsu(message: Message):
    ...
    keyboard = Keyboard()
    keyboard.add(Text("1 модуль", {"cmd": "1 модуль"}), color=KeyboardButtonColor.PRIMARY)
    keyboard.add(Text("2 модуль", {"cmd": "2 модуль"}), color=KeyboardButtonColor.PRIMARY)
    keyboard.add(Text("3 модуль", {"cmd": "3 модуль"}), color=KeyboardButtonColor.PRIMARY)
    keyboard.add(Text("4 модуль", {"cmd": "4 модуль"}), color=KeyboardButtonColor.PRIMARY)
    keyboard.row()
    
    keyboard.add(Text("5 модуль", {"cmd": "5 модуль"}), color=KeyboardButtonColor.PRIMARY)
    keyboard.add(Text("6 модуль", {"cmd": "6 модуль"}), color=KeyboardButtonColor.PRIMARY)
    keyboard.add(Text("7 модуль", {"cmd": "7 модуль"}), color=KeyboardButtonColor.PRIMARY)
    keyboard.add(Text("8 модуль", {"cmd": "8 модуль"}), color=KeyboardButtonColor.PRIMARY)
    keyboard.row()
    
    keyboard.add(Text("Меню", {"cmd": "Меню"}), color=KeyboardButtonColor.POSITIVE)
    await message.answer("1 модуль - основы программирования на Python", keyboard=keyboard)
