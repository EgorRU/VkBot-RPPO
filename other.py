...
async def update_BD(message, command):
    ...
    base.commit()
    id = cur.execute(f"SELECT id FROM registration_data WHERE id={message.peer_id}").fetchone()
    if id is None:
        cur.execute(
        ...
        base.commit()
        count = int(cur.execute(f"SELECT count FROM registration_data WHERE id={message.peer_id}").fetchone()[0])
        cur.execute("UPDATE registration_data SET count==? WHERE id=?",(count + 1, message.peer_id),)
        base.commit()
    base.close()
